...
const SRC_FILE = path.join(__dirname, '..', 'assets', 'silence-godfather.jpg');
...
async function main() {
  ...
  if (html.includes("silence:'data:image")) {
    console.log('IMG.silence already present, skipping.');
    return;
  }
  ...
  console.log('Injected IMG.silence (' + buf.length + ' bytes). New file size:', fs.statSync(GAME_FILE).size);
}
